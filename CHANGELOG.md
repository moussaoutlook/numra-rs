...
- `numra-ode`: `solve_trajectory` (uncertainty.rs) is ~42% faster after the canonical-primitive port + FD scratch-buffer optimisation. Measured on a Lotka–Volterra problem (4 params, rtol=1e-9, n=401 reps × 4 warm runs): warm median 0.085 ms post-port vs 0.147 ms pre-port. The win comes from eliminating per-RHS-call allocations that the previous private `AugmentedOdeSystem` paid (six fresh `Vec`s per evaluation); the new path reuses scratch via `RefCell` buffers on `AugmentedSystem`.
