...
- **Mass matrix as a first-class composable.** Currently mass-matrix DAE is supported in `Radau5` / `BDF` only; PDE/sensitivity coupling doesn't accept DAE form. Promoting mass-matrix to first-class in `OdeSystem` is a foundation change worth doing once a user requests it. **Update (0.1.5, §6 #14):** Partially concretized on the `ParametricOdeSystem` side. The forward-sensitivity trait gained the same four-method mass-matrix surface (`has_mass_matrix`, `mass_matrix`, `is_singular_mass`, `algebraic_indices`) plus `is_autonomous` — defaults match `OdeSystem`'s defaults so every pre-0.1.5 implementor compiles unchanged. `AugmentedSystem` now lifts the host `M` block-diagonally onto the augmented `(N · (1 + N_s))` state; `IcAsParametric` now forwards the host `OdeSystem`'s M surface into the augmented path. The broader question this bullet originally raised — whether `OdeSystem::mass_matrix`'s identity default should itself be removed (forcing every implementor to declare M-ness explicitly), or whether a `DaeSystem` peer trait should peel DAE off into its own type — is **not** answered by 0.1.5 and remains live as §7 #3.
...
### 3.8 `ParametricOdeSystem`

**Purpose:** Peer trait to `OdeSystem` for parameterised right-hand sides `f(t, y, p)`. Drives forward sensitivity (`solve_forward_sensitivity{,_with}`) and, via the private `IcAsParametric` wrapper, initial-condition sensitivity (`solve_initial_condition_sensitivity{,_with}`).

**Current signature (`numra-ode/src/sensitivity.rs:152`):**
...
pub trait ParametricOdeSystem<S: Scalar> {
    ...
    fn rhs(&self, t: S, y: &[S], dydt: &mut [S]) { /* default: dispatch to rhs_with_params(self.params()) */ }
    fn jacobian_y(&self, t: S, y: &[S], jac: &mut [S]) { /* default FD, sqrt(S::EPSILON) step */ }
    fn jacobian_p(&self, t: S, y: &[S], jp: &mut [S]) { /* default FD */ }
    fn initial_sensitivity(&self, y0: &[S], s0: &mut [S]) { /* default: zero */ }
    fn has_analytical_jacobian_y(&self) -> bool { false }
    fn has_analytical_jacobian_p(&self) -> bool { false }
    fn is_autonomous(&self) -> bool { false }                                  // 0.1.5 §6 #14
    fn has_mass_matrix(&self) -> bool { false }                                // 0.1.5 §6 #14
    fn mass_matrix(&self, mass: &mut [S]) { /* default identity */ }           // 0.1.5 §6 #14
    fn is_singular_mass(&self) -> bool { false }                               // 0.1.5 §6 #14
    fn algebraic_indices(&self) -> Vec<usize> { Vec::new() }                   // 0.1.5 §6 #14
}
```

No `Send + Sync` bound on the trait itself (§2.7). Call-site escalation via `where Sys: ParametricOdeSystem<S> + Send + Sync` is the documented pattern. The blanket `impl<T: ParametricOdeSystem<S>> ParametricOdeSystem<S> for &T` forwards every method including the M surface, so `&Sys` carries the host's declaration through `solve_forward_sensitivity` without ownership transfer.

**Design choices:**

- Layout: `J_y` row-major, `J_p` column-major, `S(t)` column-major within the per-time block. The asymmetry matches CVODES indexing, makes per-parameter sensitivity columns contiguous, and gives the column-major flattening that `AugmentedSystem` consumes in its inner loop.
- `has_analytical_jacobian_y` / `has_analytical_jacobian_p` flag pattern with a debug-build consistency-check safety net inside `AugmentedSystem` (catches "implemented analytical but forgot the flag" silently-bypassed analytical Jacobians).
- **Mass-matrix surface mirrors `OdeSystem` (§6 #14, 0.1.5).** Same flag-and-fill-buffer design (`has_mass_matrix() -> bool` plus `mass_matrix(&self, mass: &mut [S])` filling row-major), same `is_singular_mass() -> bool` and `algebraic_indices() -> Vec<usize>` peer methods, plus `is_autonomous() -> bool`. Defaults match `OdeSystem` (`false` / identity / `false` / empty / `false`), so every pre-0.1.5 `ParametricOdeSystem` implementor compiles unchanged. `AugmentedSystem`'s `OdeSystem` impl lifts the host's `n × n` `M` block-diagonally onto the augmented `(N · (1 + N_s)) × (N · (1 + N_s))` state — the `(N_s + 1)` copies on the diagonal follow from differentiating `M · y' = f(t, y, p)` with respect to each parameter, which places each variational column `∂y/∂p_k` in the same M-weighted space as `y` itself. Host algebraic indices lift correspondingly: for host index `i`, the augmented system has indices `{i, N + i, 2N + i, …, N_s · N + i}` (one per block). The lifted indices are cached on `AugmentedSystem` at construction (the `&[usize]` semantics permit per-call clones, but caching avoids per-call vector allocation in solver hot paths).
- **`IcAsParametric` wrapper (sensitivity.rs:983) forwards the M surface and `is_autonomous` inward.** Closes the inward-direction boundary-check gap surfaced during the §6 #14 audit (F-IC-SENS's deliverable-E boundary check was outward-only — verified no parameter-vocabulary leakage to the user, did not verify the wrapper preserved the host `OdeSystem`'s foundation capabilities inward). See [[feedback_wrapper_boundary_symmetry]].

**Deferred decisions:**

- **Sparse Jacobian return** (same shape as the §3.3 deferred bullet — applies to `jacobian_y` here).
- **JVP / VJP variants** (same shape as the §3.3 deferred bullet).
- **`DaeSystem` peer trait or `OdeSystem` default removal** — see §7 #3. The 0.1.5 ParametricOdeSystem M surface deliberately mirrors `OdeSystem`'s shape rather than diverging, so a future foundation pass that promotes M differently on either trait can do so coherently on both.
...
13. **2026-05-17 (commit `98ee6ca`; release: `0.1.4` Unreleased)** — F-IC-SENS + F-AUTODIFF-JY landed in `numra-ode`. **F-IC-SENS (additive)**: first-class initial-condition sensitivity entry points `solve_initial_condition_sensitivity{,_with}` returning `StateTransitionResult<S>` (`Φ(t) = ∂y(t)/∂y₀` over any `Solver`). Implemented as a private wrapper that reinterprets a plain `OdeSystem<S>` as a `ParametricOdeSystem<S>` with `n_params := n_states`, `J_p ≡ 0`, `S(t₀) = I_N`, then delegates to the existing `solve_forward_sensitivity` / `AugmentedSystem` machinery — **no fork of the variational integrator**. The IC-NO-PARAM-READ invariant (the wrapped RHS provably does not read its dummy `params()` slice, so `∂f/∂p ≡ 0` is mathematically exact and the AugmentedSystem analytical-zero-J_p flag is truthful) is pinned by a NaN/∞-dummy-params test in `ic_sensitivity_regression.rs`. The variational-route choice is correctness given the actual codebase: the AD-through-y₀ alternative requires either making `Dual<f64>: Scalar` with dual-aware step-control norms across every solver (the existing `Dual<S>::PartialOrd` is primal-only, dropping the tangent — the STM — from norm-driven step adaptation), or a fork of the integrator; both are forbidden by §2.6 / §2.7 / the composability story for this scope. Foundation-affecting because §3.3's deferred "Jacobian enum (AutoDiff | Analytical | FD) would change the trait" question is **not** preempted by this entry: the AD adapter (see below) composes below the trait shape. On §3.3's adjacent call ("the autodiff path needs to be a foundational integration, not a per-crate retrofit"): the adapter is *not* a per-crate retrofit even on a broad reading of that sentence — it implements the foundation `OdeSystem<S>` trait uniformly, so every existing `Solver<S>` consumer of `OdeSystem` benefits from AD-derived `J_y` without per-crate plumbing. The per-crate retrofit risk §3.3 warns against concentrates on the gradient-source axis (multiple call sites in `numra-optim`, `numra-fit`, etc.), which §7 #1 is rewritten to keep explicitly open. **F-AUTODIFF-JY (additive, behind cargo feature `autodiff`)**: `AutodiffJacobianSystem<S, F>` adapter implementing `OdeSystem<S>` with a `Dual<S>`-typed RHS closure; **zero allocation per call** (the dual scratch buffers are owned by the adapter behind `RefCell`), with a per-Jacobian cost of `N` Dual-RHS evaluations (one per input direction `e_j`) — same `O(N)` complexity class as the FD-default's `N+1` plain-RHS evals per Jacobian, at ~2× per-eval constant for a round-off-exact Jacobian instead of `~sqrt(ε)`-truncated. Per integrator step the adapter additionally performs 1 Dual-RHS evaluation for `OdeSystem::rhs`, the same 1 RHS evaluation any solver pays regardless of Jacobian source. Cargo-feature-gated so non-opting callers pay zero in compile time and binary size. **This entry resolves §7 #1** (autodiff integration shape) for the Jacobian-source axis: the answer committed is *per-system adapter (`OdeSystem<S>` impl bound to the system once at construction) made opt-in via cargo feature gate*. The three §7 #1 options collapse: adapter-as-`OdeSystem` is the API mechanism (a); per-system constructor is the entry-point shape (b); cargo feature is compile-time discoverability (c). §7 #1's narrower scope ("every Jacobian/gradient call site") is **not** fully closed: gradient-side wiring (numra-optim, numra-fit) remains open; only the `OdeSystem::jacobian` axis concretises. §7 #1 is rewritten in this commit to reflect the narrower remaining scope. The header SHA placeholder resolves via the §6 #12 two-commit pattern, per `feedback_spec_decision_log_anchor_pattern.md`'s fixed-point fix for the single-commit-with-amend chicken-and-egg: commit 1 (this commit) stamps the placeholder shown in the entry header above; commit 2 (a follow-up commit on the same PR) substitutes the merge SHA into that header's literal-token slot only. The substitution is contextual to the header line — the literal token appears exactly once in this entry, in the header, so identifying the substitution target is unambiguous (no global-replace footgun).

14. **2026-05-21 (commit `0a78d7e`; release: `0.1.5` Unreleased)** — F-IC-SENS-MASS-SURFACE landed in `numra-ode`. **(A) `ParametricOdeSystem` trait extended (additive, non-breaking)**: gained the four-method mass-matrix surface (`has_mass_matrix`, `mass_matrix(&self, mass: &mut [S])`, `is_singular_mass`, `algebraic_indices`) plus `is_autonomous`, all five with default impls matching `OdeSystem`'s defaults exactly (`false` / row-major identity / `false` / empty / `false`). Non-breaking because the defaults reproduce the pre-0.1.5 implicit behavior — every existing implementor (`AugmentedSystem`, `ClosureSystem`, `ParametricMOLSystem2D/3D`, `IcAsParametric`, the `&T` blanket impl, every user-defined external impl) compiles unchanged with the defaults. **(B) `AugmentedSystem<S, Sys: ParametricOdeSystem<S>>`'s `OdeSystem<S>` impl now delegates and lifts the M surface** instead of inheriting `OdeSystem`'s trait defaults. The block-diagonal lift `M_aug = block_diag(M, M, …, M)` with `(N_s + 1)` copies of the host's `M` follows from differentiating `M · y' = f(t, y, p)` with respect to each parameter: the variational column `∂y/∂p_k` satisfies `M · (∂y/∂p_k)' = J_y · (∂y/∂p_k) + J_p_{:,k}`, so each sensitivity column lives in the same M-weighted space as the state. Algebraic indices lift correspondingly: for each host index `i`, the augmented system has algebraic indices `{i, N + i, 2N + i, …, N_s · N + i}` (one per block). `is_autonomous`, `has_mass_matrix`, and `is_singular_mass` forward directly from the host; the lifted-indices `Vec<usize>` is cached on `AugmentedSystem` at construction (not recomputed per call). The `has_mass_matrix() = false` branch of the lifted `mass_matrix` fills the full augmented identity rather than the host's identity-lifted result — preserving the semantic distinction between "host has M, M happens to be identity" and "host has no M", which downstream solvers (Radau5, BDF) dispatch on. **(C) `IcAsParametric<'a, S, Sys: OdeSystem<S>>` (sensitivity.rs:983) now forwards the host `OdeSystem`'s M surface and `is_autonomous` into the now-richer `ParametricOdeSystem`** (the five methods (A) added). Combined with (B), this lifts the host's M end-to-end through `solve_initial_condition_sensitivity{,_with}` for the first time — the IC-sensitivity path now honors mass matrices.

    **Defect locus and the asymmetric boundary check.** F-IC-SENS shipped in 0.1.4 (§6 #13) with a documented deliverable-E boundary check that verified the public API surface mentions no parameter/sensitivity/p/sigma/uncertainty/measurement/application-domain concepts (outward-direction discipline: the wrapper does not leak parametric-vocabulary to the user). That check was **directional** — it did not verify the symmetric inward property: that the wrapper preserves the wrapped `OdeSystem`'s foundation-trait capabilities through to the augmented integration. The audit for this entry found `IcAsParametric` silently dropping the host's mass-matrix declaration (and `is_autonomous`) when re-presenting the system as a `ParametricOdeSystem` — a real correctness gap on 0.1.4 for the IC-sensitivity entry points, of bounded scope (only the IC-sensitivity callers; only those with non-identity or singular M). The lesson is recorded as a saved memory: wrappers between foundation types are at risk of asymmetric boundary checks by default and need explicit *both-direction* discipline (see [[feedback_wrapper_boundary_symmetry]]). Same hard-stop discipline shape as F-WEBSITE-AUDIT-GATES's preview-vs-production correction caught by the symmetric-overclaim guard — a different axis, same lesson about checking properties in *all* relevant directions.

    **Precisely-supported affected-scope claim for 0.1.4.** Verified empirically against `HEAD:numra-ode/src/sensitivity.rs:152-281` at commit `98ee6ca` (0.1.4 head): the pre-extension `ParametricOdeSystem` trait body contains exactly `n_states`, `n_params`, `params`, `rhs_with_params`, `rhs`, `jacobian_y`, `jacobian_p`, `initial_sensitivity`, `has_analytical_jacobian_y`, `has_analytical_jacobian_p` and **none** of `has_mass_matrix`, `mass_matrix`, `is_singular_mass`, `algebraic_indices`, or `is_autonomous`. Therefore: users of `solve_forward_sensitivity{,_with}` with `ParametricOdeSystem` implementors are **unaffected** by the bug — the pre-0.1.5 trait had no mass-matrix surface, so users had no way to declare M through this path; the missing delegation in `AugmentedSystem` was a *compile-time* fact (the problem couldn't be expressed), not a runtime numerical failure. Users of `solve_initial_condition_sensitivity{,_with}` with identity-M `OdeSystem` (the common ODE case): **unaffected** — the silently-dropped M was identity anyway. Users of `solve_initial_condition_sensitivity{,_with}` with **non-identity M** (e.g. FE-discretized PDE): received Φ off by an M⁻¹ factor *in the matrix-exponential's argument* — `Φ_bug(t) = expm(A·t)` instead of `Φ_truth(t) = expm(M⁻¹·A·t)`. Pinned by the pre-fix-bug numerical value `exp(-2) ≈ 0.13534` in `ic_sensitivity_mass_matrix.rs::path_b_non_identity_mass_ic_sensitivity`, derived in closed form: the test uses the scalar DAE `2y' = -y` (M = 2, A = -1), so M-blind integration runs `y' = -y` and produces `Φ_bug(t) = exp(-t)` versus truth `Φ_truth(t) = exp(-t/2)`; at t = 2, `Φ_bug = exp(-2) ≈ 0.13534`, `Φ_truth = exp(-1) ≈ 0.36788`. Scalar choice (1×1 M) keeps the falsifiability gate's pre-fix-bug value derivable in closed form, mirroring Test 4's DAE closed-form; higher-dimensional non-identity-M systems exhibit the same matrix-exponential-argument-off-by-M⁻¹ shape without scalar reduction. Revert-confirm protocol verified. Users with **singular M** (DAE): received variational integrations that treated algebraic rows as differential — wrong dynamics, not just wrong scaling — pinned by the pre-fix-bug numerical value `y₂(2) ≈ 0.73039` in `path_b_singular_mass_dae_ic_sensitivity` (the closed-form solution to `y₂' + y₂ = sin(t)` with `y₂(0) = 0`, which is what the M-blind integration actually solves). **Yank decision: do not yank 0.1.4.** Bounded scope (specific feature path × specific user category — non-identity-M or singular-M users of the IC-sensitivity entry points); academic citation record references 0.1.4 immutably (Zenodo DOI back-port landed in `47d25a9`); 0.1.5 supersedes with the documented CHANGELOG connection. 0.1.4 stands; 0.1.5 fixes.

    **§6 #13 self-consistency.** Entry #13 argued the variational route was architecturally correct over the AD-through-y₀ alternative because the latter would require either making `Dual<f64>: Scalar` with dual-aware step-control norms across every solver, or a fork of the integrator. Entry #13's original text asserted that architectural-correctness flat — it did not state a contingency. The honest framing: that claim **implicitly assumed** end-to-end M-preservation through the variational route, which 0.1.4 did not deliver; the assumption was *tacit* in the original framing and is made *explicit and satisfied* by §6 #14. The variational-vs-AD-through-y₀ decision is undisturbed. §7 #1's gradient-source axis (currently open) is unaffected by this entry; the AD-through-y₀ vs variational *Jacobian-source* axis remains closed by §6 #13.

    **Cross-references.** §3.3 "Mass matrix as a first-class composable" deferred bullet updated to reflect partial concretization. §3.8 added (was missing for `ParametricOdeSystem` — a documentation drift this entry closes); the §3.8 description includes the new M surface. §7 #3 updated to note the ParametricOdeSystem half is decided; the OdeSystem-default-removal / DaeSystem-peer-trait question remains, with the trigger split into two independent sub-conditions (see §7 #3). §7 #9 added (new): IC manifold-projection follow-up for singular-M IC sensitivity — the finite-difference IC perturbation at `fd_jacobian_y_inline` (sensitivity.rs:394) generically moves the perturbed IC off the constraint manifold for singular-M systems; 0.1.5's variational integration is now M-aware, so the post-fix state is "wrong sensitivity-w.r.t.-perturbed-IC because the perturbed IC may drift off-manifold, correctly integrated because the variational integration is now M-aware" — strictly improved, but the IC-projection question (when to project, in which direction, via which API, under which correctness criterion) is its own foundation design pass.

    **Regression discipline.** Two distinct change-classes per §2.8 visibility: one `### Added` CHANGELOG entry (the additive `ParametricOdeSystem` extension) and one `### Fixed` entry (the IC-sensitivity correctness fix, with the precisely-supported affected-scope language above split by severity — *recommended* for non-identity-M, *required* for singular-M DAE where the pre-fix result is qualitatively wrong rather than merely inaccurate). Six analytic-oracle tests pin correctness end-to-end (`numra-ode/tests/ic_sensitivity_mass_matrix.rs`): Path A non-identity-M (`path_a_non_identity_mass_param_sensitivity`, post-fix only — would not compile against 0.1.4 because the trait lacked the M surface), Path A singular-M DAE (`path_a_singular_mass_dae_param_sensitivity`, same compile-time fact); Path B non-identity-M (`path_b_non_identity_mass_ic_sensitivity`, post-fix assertion + falsifiability gate on the pre-fix `exp(-t)` value), Path B singular-M DAE (`path_b_singular_mass_dae_ic_sensitivity`, post-fix assertion + falsifiability gate on the closed-form `(1/2)(sin(t) - cos(t) + e^(-t))` algebraic-as-differential bug value); structural unit test on `AugmentedSystem`'s lift (`augmented_system_lifts_mass_surface_block_diagonally`) plus the symmetric counterpart on `IcAsParametric` (`numra-ode/src/sensitivity.rs::tests::ic_as_parametric_forwards_mass_surface`, lives in the lib because `IcAsParametric` is private) — these two complete the boundary-symmetry structural pair. The path-typed distinction is encoded in each test's source: Path A tests say "would not compile against 0.1.4 because the trait lacked the M surface" (compile-time fact); Path B tests say "compiles against 0.1.4 and exhibits the documented failure shape" (runtime fact). Revert-confirm-restore protocol applied (revert `IcAsParametric::mass_matrix` / `has_mass_matrix` / `is_singular_mass` / `algebraic_indices` to defaults → Path B tests fail with the exact predicted pre-fix-bug numerical values → restore wiring → tests pass). A seventh sanity test (`augmented_system_identity_m_default_path_unchanged`) pins the identity-M path of `AugmentedSystem::mass_matrix` so the semantic distinction between "host has no M" and "host has identity M" survives future refactors.

    **Anchor pattern.** Two-commit anchor per `feedback_spec_decision_log_anchor_pattern.md`. Commit 1 (this commit) stamps the placeholder shown in the entry header above. Commit 2 (a separate docs-only follow-up commit on the same PR, mirroring `68aab53`'s pattern from §6 #13) substitutes the merge SHA into that header's literal-token slot only. The literal placeholder token appears exactly once in this entry — in the header — so identifying the substitution target is unambiguous (no global-replace footgun); the same single-occurrence convention §6 #13 established. **Post-substitution success criterion (explicit gate, stated up front):** `grep -c 'pending' docs/architecture/foundation-specification.md` returns `0` within this entry's lines (entries elsewhere in the file may discuss the pattern in prose without using the literal placeholder), and `git rev-parse <commit-2>^` matches the SHA written into the entry header.
...
3. **Mass-matrix DAE first-class promotion (partial progress 0.1.5; OdeSystem-default-removal and DaeSystem-peer-trait remain open as two independent sub-questions).** Currently DAE works in `Radau5`/`BDF` end-to-end including sensitivity (§6 #14, 0.1.5). The `ParametricOdeSystem` half is concretized: both foundation traits (`OdeSystem` and `ParametricOdeSystem`) now expose the same four-method mass-matrix surface with identity defaults. The remaining foundation-design territory has **two independent sub-questions**, each with its own trigger:
   - **(i) `OdeSystem::mass_matrix` identity-default removal.** Should the identity default on `OdeSystem::mass_matrix` (and now `ParametricOdeSystem::mass_matrix`) be removed — forcing every implementor to declare M-ness explicitly, making the type system enforce "is this a DAE?" instead of leaving it to a `has_mass_matrix() -> bool` flag? Workspace-wide breaking change forcing every existing `OdeSystem` impl to add four trivial method bodies. **Trigger for (i):** a workspace-wide migration cost-benefit analysis (worth doing once the workspace's `OdeSystem` impl count stabilizes), *or* a third-party impl confusion report (a user surfacing that the silent identity default produced unexpected behavior in their code).
   - **(ii) `DaeSystem` peer trait.** Should the DAE shape be peeled off into a separate `DaeSystem` peer trait, with `OdeSystem` reserved for the strictly differential case? Non-breaking but introduces another foundation trait and the composition story across `OdeSystem ↔ DaeSystem` (does every `OdeSystem` impl `DaeSystem`? does `Solver` consume both?) needs designing. **Trigger for (ii):** a `Solver` impl wanting to dispatch differently on declared-DAE vs declared-ODE (currently no such solver exists in the workspace), *or* a workload where the `OdeSystem ↔ DaeSystem` composition question becomes concrete (e.g., a problem class that wants DAE structure but can't comfortably express it through `OdeSystem`'s flag-based surface).

   The triggers are independent: either sub-question can be resolved without committing to the other. A future foundation pass that addresses one and defers the other is the expected pattern.
...
9. **IC manifold-projection for singular-M sensitivity.** With §6 #14 (0.1.5), the variational integration inside `solve_initial_condition_sensitivity{,_with}` honors the host's mass matrix end-to-end. The finite-difference Jacobian construction at `sensitivity.rs:394` (`fd_jacobian_y_inline`) still perturbs each `y[j]` by `h = sqrt(S::EPSILON) * (1 + |y_j|)` *without* projecting the perturbed initial condition back onto the constraint manifold defined by the algebraic rows. For singular-M (DAE) systems, this generically moves the perturbed IC off the constraint manifold; the resulting Φ is "the sensitivity given a manifold-inconsistent IC perturbation, correctly integrated against M-aware variational dynamics" — strictly improved over pre-0.1.5 (where the variational dynamics themselves were wrong), but still not the manifold-projected sensitivity a DAE user with explicit constraint-projection expectations would want. **Four design axes**, each its own design call:
   - **(a) When to project.** Always (on every IC perturbation); opt-in (a `SolverOptions` flag or a `project_at_ic = true` per-call argument); opt-out (default on, with an explicit opt-out for users who want the manifold-inconsistent perturbation). Each policy has different ergonomics and different surprise profiles.
   - **(b) In which direction.** The projection direction is non-trivial for non-trivial constraints: project along the algebraic null-space of `M`'s singular block; project along a user-supplied projector; project along an autodiff-derived manifold tangent at the unperturbed IC. The "right" direction depends on which post-projection invariant the user actually wants preserved.
   - **(c) Via which API surface.** A `project_to_manifold(&self, y: &mut [S])` method on `OdeSystem` / `ParametricOdeSystem` (lets the user define the projection); an option on `SolverOptions` (lets the framework pick a default projector); a separate `solve_initial_condition_sensitivity_with_projection` entry point (avoids changing the existing API).
   - **(d) Correctness criterion.** Linear constraints have a clean projector (orthogonal projection onto the null-space of the constraint Jacobian, computable in one linear solve); nonlinear constraints require nonlinear projection (Newton on the constraint manifold) with its own tolerance, convergence handling, and failure modes. Different correctness criteria for the linear and nonlinear cases may be appropriate — a linear-only solution would prematurely close this entry while nonlinear DAEs remain unaddressed.

   Defer; surface as a focused design pass when a DAE user requests the projected sensitivity, or when the broader §7 #3 promotion (either sub-question) lands and forces the projection question through the type system. Until then, the singular-M sensitivity tests in `numra-ode/tests/ic_sensitivity_mass_matrix.rs` explicitly comment the IC-projection caveat and assert only on the state trajectory, not on Φ.
